...
module "metadata" {
  ...
  metadata = {
    "enable-guest-attributes" = contains(local.project_services, "containeranalysis.googleapis.com") && var.enable_osconfig ? "TRUE" : "FALSE"
    "enable-os-inventory"     = contains(local.project_services, "containeranalysis.googleapis.com") && var.enable_osconfig ? "TRUE" : "FALSE"
